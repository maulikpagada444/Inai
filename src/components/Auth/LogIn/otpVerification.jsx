  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await axios.post(
        `${BASE_URL}/auth/login/forgot-password/verify-otp/`, // 🔹 use BASE_URL
        {
          email,
          otp: enteredOtp,
        },
      ...
      if (res.data.status) {
        ...
        setStep(4); // go to next step, maybe reset password screen
      } else {
  ...
  return (
    <div className="text-white z-10 relative flex justify-center items-center h-screen">
      ...
      >
        ...
        {otpError && (
          <p className="text-red-500 sm:text-sm text-[10px] text-center">{otpError}</p>
        )}
